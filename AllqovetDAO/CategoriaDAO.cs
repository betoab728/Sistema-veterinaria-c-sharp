...
namespace AllqovetDAO
{
    public class CategoriaDAO : ICategoria, IDisposable
    {
        ...
        public DataTable Buscar(Categoria categoria)
        {
            using (MySqlConnection cn = new MySqlConnection(cnx))
            {
                using (MySqlCommand cmd = new MySqlCommand("sp_BuscarCategoria", cn))

                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("pBuscar", categoria.Nombre);
        ...
        public DataTable Listar()
        {
            using (MySqlConnection cn = new MySqlConnection(cnx))
            {
                using (MySqlCommand cmd = new MySqlCommand("sp_ListarCategoria", cn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cn.Open();
                    using (MySqlDataAdapter da = new MySqlDataAdapter(cmd))
